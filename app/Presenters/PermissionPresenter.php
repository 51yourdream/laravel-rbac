    ...
    public function menus()
    {
        ...
        if($menus) {
            ...
            foreach ($menus as $menu) {
                ...
                $html .= sprintf('<a href="%s">%s <span>%s</span></a>', $href, $menu['icon_html'], $menu['display_name']);
                ...
                foreach ($menu['sub'] as $sub) {
                    ...
                    $icon = $sub['icon_html'] ? $sub['icon_html'] : '<i class=""></i>';

                    if(!Auth::guard('admin')->user()->is_super && !Auth::guard('admin')->user()->can($sub['name'])){
                        $html .= sprintf('<li class=" hide"><a href="%s">%s %s</a></li>', $href, $icon, $sub['display_name']);
                    }else{
                        $html .= sprintf('<li><a href="%s">%s %s</a></li>', $href, $icon, $sub['display_name']);
                    }
                }
                $html .= '</ul>';
                $html .= '</li>';
